...
use digest::generic_array::GenericArray;
use elliptic_curve::{sec1::EncodedPoint, Curve};
    ...
    SignatureSize<C>: ArrayLength<u8>,
{
    ...
    #[allow(warnings)]
    #[cfg(all(target_os = "zkvm", target_vendor = "succinct"))]
    fn recover_from_prehash_zkvm(r_x_bytes: &[u8], r_y_odd: bool, s: NonZeroScalar<C>, z: <C as CurveArithmetic>::Scalar) -> Result<Self> {
        // A non quadratic residue mod `SECP256K1_ORDER`
        const SECP256K1_NQR: [u8; 32] = {
            let mut nqr = [0u8; 32];
            nqr[31] = 5;
            nqr
        };
        // A non quadratic residue mod `SECP256R1_ORDER`
        ...
        if C::ORDER == <C as Curve>::Uint::decode_field_bytes(GenericArray::from_slice(&SECP256K1_ORDER)) {
            a = C::Scalar::from_repr(GenericArray::clone_from_slice(&SECP256K1_A)).unwrap();
            b = C::Scalar::from_repr(GenericArray::clone_from_slice(&SECP256K1_B)).unwrap();
            nqr = C::Scalar::from_repr(GenericArray::clone_from_slice(&SECP256K1_NQR)).unwrap(); 
            curve_id = 1;
        } else if C::ORDER == <C as Curve>::Uint::decode_field_bytes(GenericArray::from_slice(&SECP256R1_ORDER)) {
            a = C::Scalar::from_repr(GenericArray::clone_from_slice(&SECP256R1_A)).unwrap();
            b = C::Scalar::from_repr(GenericArray::clone_from_slice(&SECP256R1_B)).unwrap();
            nqr = C::Scalar::from_repr(GenericArray::clone_from_slice(&SECP256R1_NQR)).unwrap();
            curve_id = 2;
        } else {
            unimplemented!("Unsupported curve");
        };
       
        let r = C::Scalar::from_repr(GenericArray::clone_from_slice(r_x_bytes)).unwrap();        
        // The first step of the recovery is to decompress the R point, whose x-coordinate is given
        // by r_x_bytes.
        let alpha = r * r * r + (a * r) + b;
        ...
        buf[33..65].copy_from_slice(&alpha.to_repr());
        ...
        if !status {
            ...
            // So we need to constrain the by proving alpha is non square.
            let root_bytes = sp1_lib::io::read_vec();
            let root = C::Scalar::from_repr(GenericArray::clone_from_slice(&root_bytes)).unwrap();
        ...
        // This should be the big endian representation of the inverse of r. 
        let point_bytes = sp1_lib::io::read_vec();
        ...
        let pk_bytes = match curve_id {
            ...
            1 => {
                let p = Secp256k1Point::multi_scalar_multiplication(
                    ...
                    &u2_le_bits,
                    Secp256k1Point::from_le_bytes(&point_bytes),
                )
                    .unwrap();
            ...
            2 => {
                let p = Secp256r1Point::multi_scalar_multiplication(
                    ...
                    &u2_le_bits,
                    Secp256r1Point::from_le_bytes(&point_bytes),
                )
                    .unwrap();
